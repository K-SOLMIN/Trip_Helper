...
import {
  buildGeneratedTravelData,
  getDayStops,
  getRouteInfo,
  getRouteSegment,
  haversineMeters,
  formatDistance,
  lookupEmergencyNumber,
  heroImageForDestination,
  makeFormatters,
  getTransportPlan,
  getTransitDemoOptions,
  getTransitDetailOptions,
  transitPanelKey,
  modeResultKey,
  getTotalSpent,
  getCurrentDayNumber,
  getDailyBudgetWon,
  getCurrentDayExpenses,
  getCurrentDaySpent,
  getTodayAvailableBudget,
  getCategoryBreakdown,
  parseBudgetWon,
  normalizeSavedExpense,
  fetchSavedExpenses,
  persistExpense,
  deleteExpense,
  fetchExchangeRate,
  ...
  renderSafeRouteMap,
  getGeneratedPlanId,
  readGeneratedPlanResult,
  BUDGET_CATEGORIES,
  GOOGLE_MAP_SCRIPT_ID,
  GOOGLE_MAP_SCRIPT_SRC,
...
const PODCAST_ICON_URL =
  "https://cdn-icons-gif.flaticon.com/15748/15748293.gif";
const PODCAST_ICON_STATIC_URL =
  "https://cdn-icons-png.flaticon.com/512/15748/15748293.png";
    ...
    <div>
      ...
      {(translated || error) && (
        <>
          ...
          {pronunciation && (
            <div className="mi-pronunciation">[ {pronunciation} ]</div>
          )}
        </>
      )}
    </div>
...
function ImageTranslateModal({
  ...
  async function startUpload(nextFile = file) {
    ...
    const timer = window.setInterval(() => {
      current = Math.min(
        92,
        current + Math.max(2, Math.round((92 - current) * 0.12)),
      );
      setProgress(current);
    }, 240);
  ...
  const podcastText =
    result?.podcastScript || result?.summary || result?.translated;
  ...
  const summaryText = String(
    result?.summary || result?.translated || "번역 결과가 없습니다.",
  )
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean)
  ...
  return (
    <div className="mi-image-translate">
      ...
      {file && (
        <div
          className={`mi-image-processing${status === "done" ? " done" : ""}`}
        >
          {status === "done" && preview ? (
      ...
      {result && (
        <div className="mi-image-result">
          <div className="mi-image-result-title">{titleText}</div>
...
function budgetCategoryForStop(stop) {
  const text =
    `${stop?.name || ""} ${stop?.badge || ""} ${stop?.tags?.join(" ") || ""}`.toLowerCase();
  if (
    stop?.kind === "meal" ||
    /식사|점심|저녁|아침|restaurant|cafe|bar|food/.test(text)
  )
    return "meal";
  if (/쇼핑|shopping|shop|market|mall|기념품/.test(text)) return "shop";
  if (
    stop?.kind === "spot" ||
    /입장|티켓|ticket|museum|gallery|전망대|투어/.test(text)
  )
    return "entry";
  return null;
}
...
function expenseCategoryForStop(stop) {
  ...
  if (
    /택시|버스|지하철|기차|공항|이동|교통|taxi|bus|train|metro|subway|airport|transfer/.test(
      text,
    )
  )
    return "transport";
...
function mustVisitTextFromResult(result) {
  return (
    result?.tripInfo?.mustVisit ||
    (Array.isArray(result?.tripInfo?.places)
      ? result.tripInfo.places.join(", ")
      : "")
  );
}
...
export default function AiTravelDurationView() {
  ...
  const [routeCard, setRouteCard] = useState({
    title: "선택한 이동 구간",
    desc: "이동수단을 선택하면 실제 시간·거리가 표시됩니다.",
  });
  ...
  const selectedBudgetCategory =
    BUDGET_CATEGORIES.find((category) => category.key === expCat) ||
    BUDGET_CATEGORIES[0];
  ...
  const currentDayExpenses = getCurrentDayExpenses(
    expenses,
    schedule,
    activeIdx,
  );
  ...
  const todayAvailable = getTodayAvailableBudget(
    total,
    expenses,
    schedule,
    activeIdx,
  ...
  const remainingAdjustableCost = adjustmentCandidates.reduce(
    (sum, item) => sum + item.estimateWon,
    0,
  );
  const budgetRisk =
    dailyBudgetWon > 0 &&
    remainingAdjustableCost > 0 &&
    todayAvailable < remainingAdjustableCost;
  ...
  const fatigueRingOffset =
    fatigueRingCirc - (fatigueVal / 10) * fatigueRingCirc;
  const fatigueRingColor =
    fatigueVal >= 8
      ? "var(--rose)"
      : fatigueVal >= 6
        ? "var(--amber)"
  ...
  const getActiveEmergencyPoint = useCallback(() => {
    ...
    const points = selectedRoute.filter(
    ...
    return {
      ...
      routeLabel:
        points.length > 1 ? "선택 경로 중간 지점" : "현재 활성 카드 위치",
    };
  ...
  // ── 초기화: 여행 데이터 로드 ───────────────────────────────
  useEffect(() => {
    buildGeneratedTravelData().then((data) => {
  ...
  useEffect(() => {
    ...
    fetchSavedExpenses(planId)
      .then((saved) => {
        if (Array.isArray(saved)) setExpenses(saved.map(normalizeSavedExpense));
      })
      .catch(() => {});
  }, [travelData]);
  ...
  useEffect(() => {
    if (!travelData) return;
    const destination =
      readGeneratedPlanResult()?.tripInfo?.country || travelData.destination;
  ...
  useEffect(() => {
    ...
    const first = greedySelect(
      nonMustVisit.length ? nonMustVisit : adjustmentCandidates,
    );
    if (first.covered) {
    ...
    if (nonMustVisit.length && mustVisit.length) {
      const allSorted = [...adjustmentCandidates].sort(
        (a, b) => b.estimateWon - a.estimateWon,
      );
      let accumulated = 0;
      const indexes = [];
  ...
  useEffect(() => {
    ...
    fetch(
      `${base}/api/safety/incidents?lat=${stop.lat}&lng=${stop.lng}&radius=2&days=90`,
      ...
      .then((data) => {
        if (!data) return;
        const icon =
          data.level === "warn" ? "⚠️" : data.level === "caution" ? "🔶" : "✅";
        const label =
          data.level === "warn"
            ? "위험 구간"
            : data.level === "caution"
  ...
  function handleAddExpense() {
    ...
    const over =
      expCat === "meal" && total > 0 && totalSpent + amountKrw > total;
    const expense = {
      name,
      cat: expCat,
      day: currentDayNumber,
      itemIndex: activeStopIdx,
      source: "manual",
    ...
    if (over) {
      ...
      showToast(
        "⚠️",
        `${formatLocalAmount(amountLocal)} 식사 초과`,
        "예산 범위 내 근처 식당으로 재조회할까요?",
        "warn",
        [
      ...
      const msg = total
        ? `오늘 남은 예산 ${formatExpense(Math.max(0, todayAvailable - amountKrw))}`
        : "카테고리 비중이 업데이트되었습니다";
  ...
  const handleRebudgetDay = useCallback(async () => {
    ...
    try {
      ...
      if (result?.planData) {
        ...
        setActiveStopIdx(
          Math.min(
            activeStopIdx,
            result.day?.items?.length
              ? result.day.items.length - 1
              : activeStopIdx,
          ),
        );
      ...
      showToast(
        "✓",
        "예산 맞춤 재조정 완료",
        result?.summary || "오늘 남은 일정이 예산에 맞게 조정되었습니다.",
        "ok",
      );
    } catch (err) {
      showToast(
    ...
    activeIdx,
    activeStopIdx,
    selectedAdjustmentIndexes,
    selectedAdjustmentCost,
    todayAvailable,
    remainingAdjustableCost,
    showToast,
  ]);
  ...
  const handleBudgetSave = useCallback(async () => {
    ...
    const newTotal =
      budgetEditMode === "daily" ? inputWon * (nights || 1) : inputWon;
    if (newTotal > 100_000_000) {
      const maxMan =
        budgetEditMode === "daily"
          ? Math.floor(10000 / (nights || 1)).toLocaleString()
          : "10,000";
      showToast(
        "⚠",
        "예산이 너무 큽니다",
        `최대 ${maxMan}만원(1억)까지 입력 가능합니다.`,
        "warn",
      );
      return;
    }
    ...
    try {
      ...
      showToast(
        "✓",
        "예산 업데이트 완료",
        `총 예산이 ${Math.round(newTotal / 10000).toLocaleString()}만원으로 변경되었습니다.`,
        "ok",
      );
    } catch {
      showToast(
    ...
    async (action) => {
      switch (action) {
        ...
        case "safeRoute": {
          ...
          try {
            const data = await apiGet(
              `/safety/incidents?lat=${lat}&lng=${lng}&radius=2&days=90`,
            );
            setSafetyData(data);
            const levelIcon =
              data.level === "warn"
                ...
                : data.level === "caution"
                  ...
                  : "✅";
            setRouteCard({
              title: `${levelIcon} 안전 분석: ${currentStop.name || "현재 구간"}`,
              desc: data.safeRouteDesc,
            });
            if (data.level === "warn" || data.level === "caution") {
              renderSafeRouteMap(
                "liveMap",
                mapState(),
                ({ extraMin, hasAlternative }) => {
                  if (hasAlternative && extraMin !== null) {
                    const extraLabel =
                      extraMin > 0
                        ? ` · 우회 시 +${extraMin}분 추가`
                        : extraMin < 0
                          ? ` · 우회 시 ${Math.abs(extraMin)}분 단축`
                          : " · 소요 시간 동일";
                    setRouteCard((prev) => ({
                      ...prev,
                      desc: prev.desc + extraLabel,
                    }));
                  } else if (!hasAlternative) {
                    setRouteCard((prev) => ({
                      ...prev,
                      desc: prev.desc + " · 대안 경로 없음, 주의하며 이동",
                    }));
                  }
                },
              );
            ...
            setRouteCard({
              title: "안전 우회 경로",
              desc: "안전 데이터를 불러오지 못했습니다.",
            });
            showToast(
              "🛡",
              "안전 우회",
              "데이터 조회 실패, 잠시 후 다시 시도해주세요.",
              "warn",
            );
          } finally {
        ...
        case "applyMeal":
          setMealRerouteOpen(false);
          setRouteCard({
            title: `식당 변경: 평균 ${formatEurAsKrw(19)} 타파스 바 적용`,
            desc: "루트 420m 이내 · 관광 순서 유지됨",
          });
          showToast(
            "✓",
            "식당 변경 적용",
            "기존 관광 루트와 크게 벗어나지 않는 대체 식당으로 조정되었습니다.",
            "ok",
          );
          break;
        case "fatigueReroute": {
          ...
          const fatiguePt =
            fatigueStop?.lat && fatigueStop?.lng
        ...
        case "applyFatigue": {
          ...
          if (fatigueVal >= 7) {
            ...
            setRouteCard({
              title: "피로도 루트 적용됨",
              desc: "남은 구간 택시 이동으로 전환됩니다.",
            });
          }
          if (selectedCafeKey) {
            const allPlaces = (fatigueCafes?.groups || []).flatMap(
              (g) => g.places,
            );
            ...
            if (cafe) {
              setRestStop({
                ...
                cost: "",
                tags: ["휴식 추가됨", cafe.durationText || ""].filter(Boolean),
                safety: "safe",
                lat: cafe.lat,
                lng: cafe.lng,
                isInserted: true,
        ...
        case "openEmergencyMap":
          if (emergencyMapUrl)
            window.open(emergencyMapUrl, "_blank", "noopener");
          break;
        case "_dismiss":
          setToast((t) => ({ ...t, show: false }));
          break;
  ...
  // ── 트랜짓 경로 요청 ──────────────────────────────────────
  const handleTransitRequest = useCallback(
    (stopIdx) => {
      if (!day) return;
  ...
  return (
    <div className="ai-travel-duration-page">
        ...
        <div className="topbar-in">
          ...
          <div className="topbar-trip">
            ...
            <span>
              ...
              {currentCityData?.title || travelData.routeText}
            </span>
            ...
            <div className="budget-badge">
              ...
              <div className="budget-nums">
                <span className="budget-spent">
                  {formatExpense(totalSpent)}
                </span>
                <span className="budget-total">
                  / {total ? formatKrw(total) : "₩0"}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="sec-head">
              <div>
                <div className="sec-kicker">
                  DAY {String(day?.day || 1).padStart(2, "0")} · {statusLabel}
                </div>
                <h2>{currentCityData?.title || "일정을 불러오는 중"}</h2>
                <p className="sec-desc">
                  {currentCityData?.desc ||
                    "생성된 여행 일정이 있으면 이곳에 오늘의 동선이 표시됩니다."}
                </p>
              </div>
              <button
              ...
              <div className="tl">
                {displayStops.map((stop, i) => {
                  ...
                  const activeDetailStep =
                    i === activeStopIdx ? activeTransitStepIdx : null;
                  const activeMode =
                    i === activeStopIdx ? selectedTravelMode : "";
                  ...
                  return (
                    <div key={i} className="tl-node">
                      ...
                      <div className="tl-axis">
                        <span
                          className={`tl-dot ${stop.kind}${i === activeStopIdx ? " now" : ""}`}
                        ></span>
                      </div>
                      <div>
                        <div
                        ...
                        >
                          <div className="tl-card-top">
                            <h3>{stop.name}</h3>
                            <span className={`tl-badge ${stop.kind}`}>
                              {stop.badge}
                            </span>
                          </div>
                          ...
                          <div className="tl-tags">
                            ...
                            {stop.cost && (
                              <span className="tl-tag cost">
                                예상 지출 {localizeMoneyText(stop.cost)}
                              </span>
                            )}
                            {stop.safety === "warn" && (
                              <span className="tl-tag warn">⚠ 야간 주의</span>
                            )}
                          </div>
                            ...
                            <div className="tl-route-flow">
                              ...
                              <div className="tl-route-mid">
                                ...
                                <div className="tl-transport-main">
                                  <span className="tl-transport-label">
                                    {route.label}
                                  </span>
                                  <div className="tl-transport-rec">
                                    {transport.rec}{" "}
                          ...
                          {stop.mealReroute && (
                            <div
                              className={`reroute-drop${mealRerouteOpen ? " open" : ""}`}
                            >
                              <strong>식당 대체 후보</strong>
                              <p>
                                루트 420m 이내, 평균 {formatEurAsKrw(19)} 타파스
                                바로 변경.
                              </p>
                              <div className="reroute-acts">
                                <button
                                  className="rd-yes"
                                  onClick={() => handle("applyMeal")}
                                >
                                  적용
                                </button>
                                <button
          ...
          <div className="sec budget-sec">
            <div className="sec-head">
              ...
              <div className="budget-head-summary">
                <div
                  className={`budget-head-card${total && todayAvailable < 0 ? " budget-head-card--over" : ""}`}
                >
                  <span>
                    {total && todayAvailable < 0
                      ? "오늘 예산 초과"
                      : "오늘 사용 가능"}
                  </span>
                  <strong>
                    {total
                      ? formatExpense(Math.abs(todayAvailable))
                      : "예산 미설정"}
                  </strong>
                </div>
              ...
              <div className="budget-cols">
                <div className="b-block budget-edit-block">
                  <div className="budget-edit-summary">
                    <div className="budget-edit-summary-left">
                      ...
                      <span className="budget-edit-amount">
                        {total ? formatKrw(total) : "미설정"}
                      </span>
                      {total > 0 && schedule.length > 0 && (
                        <span className="budget-edit-daily-hint">
                          · 하루{" "}
                          {formatKrw(Math.round(total / schedule.length))}
                        </span>
                      )}
                    </div>
                    <button
                  ...
                  <div
                    className={`budget-edit-panel${budgetEditValue !== null ? " open" : ""}`}
                  >
                      ...
                      <div className="budget-edit-row">
                        <div className="budget-edit-input-wrap">
                          <input
                            ...
                            max={
                              budgetEditMode === "daily"
                                ? Math.floor(10000 / (schedule.length || 1))
                                : 10000
                            }
                            placeholder={
                        ...
                        {budgetEditMode === "daily" &&
                          ...
                          schedule.length > 0 && (
                            <div className="budget-edit-calc">
                              ...
                              {formatKrw(
                                Math.round(
                                  parseFloat(budgetEditValue || 0) * 10000,
                                ) * schedule.length,
                              )}
                            </div>
                          )}
                ...
                {budgetRisk && (
                  <div className="b-block budget-adjust-panel">
                    <div className="budget-adjust-head">
                      ...
                      <strong className="budget-adjust-total">
                        {formatExpense(selectedAdjustmentCost)}
                      </strong>
                    </div>
                    <p className="budget-adjust-desc">
                      남은 예산으로 오늘 일정 소화가 어려워요. 저녁 식사를 저가
                      옵션으로 바꾸거나 쇼핑 일정을 줄이는 걸 추천합니다.
                    </p>
                    <div className="budget-adjust-list">
                      {adjustmentCandidates.map((candidate) => (
                        <label
                          key={`${candidate.index}-${candidate.stop.name}`}
                          className={`budget-adjust-item${selectedAdjustmentIndexes.includes(candidate.index) ? " selected" : ""}`}
                        >
                          <input
                            type="checkbox"
                            checked={selectedAdjustmentIndexes.includes(
                              candidate.index,
                            )}
                            onChange={(event) => {
                              setRebudgetChangelog([]);
                              setSelectedAdjustmentIndexes((prev) =>
                          ...
                          <div>
                            <div className="budget-adjust-item-title">
                              ...
                              {candidate.mustVisit ? (
                                <span className="budget-adjust-badge keep">
                                  유지 권장
                                </span>
                              ) : budgetShortfall > 0 &&
                                candidate.estimateWon >=
                                  budgetShortfall * 0.5 ? (
                                <span className="budget-adjust-badge">
                                  조정 추천
                                </span>
                              ) : null}
                            </div>
                    ...
                    {adjustmentInsufficient && (
                      <div className="budget-adjust-warning">
                        남은 일정을 모두 조정해도 예산 초과를 완전히 해소하기
                        어렵습니다. 예산을 늘려주세요.
                      </div>
                    )}
                    <div className="budget-adjust-footer">
                      <button
                        type="button"
                        onClick={handleRebudgetDay}
                        disabled={
                          rebudgeting || selectedAdjustmentIndexes.length === 0
                        }
                      >
                ...
                <div className="b-block exp-block">
                  ...
                  <div className="exp-form">
                    ...
                    <div
                      className={`exp-field exp-field-category${catPickerOpen ? " open" : ""}`}
                    >
                      <span>분류</span>
                      <button
                        type="button"
                      ...
                      {catPickerOpen && (
                        <div className="exp-cat-popover">
                          {BUDGET_CATEGORIES.map((category) => (
                            ...
                            >
                              <span
                                className="exp-cat-option-icon"
                                style={{ background: category.color }}
                              >
                                {category.icon}
                              </span>
                              <span className="exp-cat-option-text">
                                {category.label}
                              </span>
                            </button>
                          ))}
                        ...
                        <div key={ei} className="exp-log-item">
                          <div className="exp-log-left">
                            <span
                              ...
                              style={{
                                background: BUDGET_CATEGORIES.find(
                                  (c) => c.key === e.cat,
                                )?.color,
                              }}
                            ></span>
                          ...
                          <div className="exp-log-right">
                            <span
                              className={`exp-log-amt${e.over ? " over" : ""}`}
                            >
                              {formatExpenseLogAmount(e)}
                            </span>
                            <button
                              className="exp-log-del"
                  ...
                  <div
                    className={`meal-reroute-panel${mealRerouteOpen ? " open" : ""}`}
                  >
                    <strong>예산 초과 — 식당 대체 후보</strong>
                    <p>
                      현재 루트 480m 이내, 평균 ₩26,460-₩32,340 식당으로만
                      재조회합니다. 관광 순서는 유지됩니다.
                    </p>
          ...
          <div className="map-wrap">
            <div className="map-hd">
              ...
              <div className="map-hd-actions">
                <button
                  ...
                  onClick={() => handle("openMapModal")}
                  aria-label="지도 확대"
                >
                  ⤢
                </button>
                <button
                  className="map-hd-btn"
                  onClick={() => handle("routeGuide")}
                >
                  경로 안내
                </button>
              </div>
            ...
            <div className="map-frame" id="liveMap">
              {!mapReady && (
                <div className="map-fallback">
                  Google Maps API 키 연결 시<br />
                  숙소·관광지·식당·야간 안전 경로 표시
                </div>
              )}
            </div>
            <div className="map-route-card">
              ...
              {safetyBadge && !safetyData && (
                <span
                  className={`safety-badge safety-badge--${safetyBadge.level}`}
                >
                  {safetyBadge.icon} {safetyBadge.label} · 안전 우회 버튼으로
                  경로 변경 가능
                </span>
              )}
            </div>
            <div className="map-btns">
              ...
              <button className="map-btn sec" onClick={() => handle("restore")}>
                원래 루트
              </button>
            </div>
          </div>
                ...
                <div className="wx-bg">
                  ...
                  <div>
                    <div className="wx-num">
                      {weatherData.afternoon?.temp != null
                        ? `${weatherData.afternoon.temp}°`
                        : "--°"}
                    </div>
                    <div className="wx-cond">
                      {weatherData.weatherLabel}
                      {weatherData.precipProb > 0
                        ? ` · 강수 ${weatherData.precipProb}%`
                        : ""}
                    </div>
                  </div>
                </div>
              ...
              <div className="fc-ring-wrap">
                <svg width="64" height="64" viewBox="0 0 64 64">
                  ...
                  <circle
                    ...
                    r="24"
                    fill="none"
                    stroke={fatigueRingColor}
                    strokeWidth="7"
                    strokeDasharray="150.8"
                    strokeDashoffset={fatigueRingOffset}
                    strokeLinecap="round"
                  />
                </svg>
      ...
      <div className={`toast${toast.show ? " show" : ""} ${toast.type}`}>
        ...
        <div className="toast-actions">
          {toast.actions.map((a, ai) => (
            <button
              key={ai}
              className={`ta-btn${a.primary ? " primary" : ""}`}
      ...
      >
        <div
          className={`modal-box${activeModalKey === "imageTranslate" ? " image-translate-modal" : ""}`}
        >
          ...
          <div className="modal-footer">
            ...
            <button className="mf ghost" onClick={closeModal}>
              닫기
            </button>
            <button
              className="mf primary"
              onClick={() => {
                if (activeModalKey === "album") saveAlbumMemory();
                else closeModal();
              }}
            >
              확인
      ...
      >
        <div className="modal-box fatigue-modal-box">
          ...
          <div className="fatigue-modal-body">
            ...
            <div className="fm-section">
              ...
              {fatigueVal >= 7 ? (
                <div className="fm-taxi-desc">
                  피로도 <strong>{fatigueVal}/10</strong> — 남은{" "}
                  {dayStops.length - activeStopIdx - 1}개 구간을{" "}
                  <strong>전부 택시</strong>로 전환합니다.
                  <span className="fm-badge high">강력 권장</span>
                </div>
              ) : fatigueVal >= 5 ? (
                <div className="fm-taxi-desc">
                  피로도 <strong>{fatigueVal}/10</strong> — 남은 구간 중 도보
                  이동을 <strong>택시로 전환</strong>을 권장합니다.
                  <span className="fm-badge mid">권장</span>
                </div>
              ) : (
                <div className="fm-taxi-desc fm-taxi-ok">
                  피로도 <strong>{fatigueVal}/10</strong> — 현재 컨디션은
                  양호합니다. 이동수단 변경이 필요하지 않아요.
                </div>
              )}
            </div>
            ...
            <div className="fm-section">
              ...
              <p className="fm-section-desc">
                현재 일정 바로 다음에 카페 휴식을 추가합니다. 하나를 선택하거나
                건너뛰세요.
              </p>
              {fatigueCafeLoading ? (
                ...
                fatigueCafes.groups
                  ...
                  .map((group) => (
                    <div key={group.key}>
                      <div className="fm-cafe-group-title">
                      ...
                      {group.places.map((place) => (
                        <div
                          ...
                          onClick={() =>
                            setSelectedCafeKey((prev) =>
                              prev === place.id ? null : place.id,
                            )
                          }
                        >
                          <div className="fm-cafe-check">
                            {selectedCafeKey === place.id ? "✓" : ""}
                          </div>
                          <div className="fm-cafe-info">
                            ...
                            <div className="fm-cafe-meta">
                              ...
                              {place.openNow === true && (
                                <span className="wx-open">영업 중</span>
                              )}
                              {place.openNow === false && (
                                <span className="wx-closed">영업 종료</span>
                              )}
                            </div>
                          </div>
                        </div>
          ...
          <div className="modal-footer">
            <button
              className="mf ghost"
              onClick={() => setFatigueModalOpen(false)}
            >
              취소
            </button>
            <button
              className="mf primary"
              onClick={() => handle("applyFatigue")}
            >
              {fatigueVal >= 5 ? "택시 전환" : ""}
              {selectedCafeKey
                ? fatigueVal >= 5
                  ...
                  : "카페 추가"
                : ""}
              {fatigueVal < 5 && !selectedCafeKey ? "확인" : ""}
            </button>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        onClick={(e) => {
          if (e.target.classList.contains("overlay")) setIndoorPanelOpen(false);
        }}
      >
        <div className="modal-box indoor-modal-box">
          ...
          <div className="indoor-modal-body">
            {indoorLoading ? (
              ...
              indoorPlaces.groups
                ...
                .map((group) => (
                  <div key={group.key} className="wx-indoor-group">
                    <div className="wx-indoor-group-title">
                      {group.icon} {group.label}
                    </div>
                    {group.places.map((place) => (
                      <a
              ...
              <div className="wx-indoor-empty">
                주변 2km 내 실내 장소를 찾지 못했어요.
              </div>
            )}
          </div>
          <div className="modal-footer">
            <button
              className="mf primary"
              onClick={() => setIndoorPanelOpen(false)}
            >
              닫기
            </button>
          </div>
      ...
      <div
        ...
        onClick={(e) => {
          if (e.target.classList.contains("overlay")) setMapModalOpen(false);
        }}
      >
      ...
      <>
        <div className="city-summary">
          <div className="city-summary-top">
            <div className="city-summary-day">
              D{String(activeDay).padStart(2, "0")} <span>/ {totalDays}</span>
            </div>
            <div className="city-summary-copy">전체 일정</div>
          </div>
          <div
            className="city-progress"
            aria-label={`여행 진행률 ${progress}%`}
          >
            <div
...
function ModalContent({
  ...
  useEffect(() => {
    ...
    if (point) {
      fetchEmergencyNearbyInfo(point, EMERGENCY_RADIUS_METERS)
        .then((data) => {
          setEmergencyMapUrl(
            data.mapUrl ||
              `https://www.google.com/maps/search/emergency+services/@${point.lat},${point.lng},14z`,
          );
          const groups = data.groups || [];
          const summary = groups
            .map((g) => `${g.label} ${g.places?.length || 0}`)
            .join(" · ");
          const rows = groups
        ...
        .catch(() => {
          setNearbyHtml(
            '<div class="mi-row"><strong>📍 주변 응급시설</strong><span class="mi-muted">정보를 불러올 수 없습니다</span></div>',
          );
          setNearbyLoading(false);
        });
    } else {
      setNearbyHtml(
        '<div class="mi-row"><strong>📍 주변 응급시설</strong><span class="mi-muted">현재 위치 좌표가 없습니다</span></div>',
      );
      setNearbyLoading(false);
    }
    ...
    if (destination) {
      fetchConsulateInfo(destination)
        ...
        .catch(() => {
          setConsulateHtml(
            '<div class="mi-row"><strong>🏛 한국 영사관</strong><span class="mi-muted">정보를 불러올 수 없습니다</span></div>',
          );
        })
  ...
  function loadFile(file) {
    ...
    fetch(`${apiBase}/api/memories/upload`, {
      ...
      .then((r) => r.json())
      .then((data) => setAlbumPhotoUrl(data.url || ""))
      .catch(() => setAlbumPhotoUrl(""));
  }
  ...
  if (modalKey === "translate")
    return <TranslateModal destination={travelData?.destination || ""} />;

  if (modalKey === "imageTranslate")
    return (
      <ImageTranslateModal
  ...
  if (modalKey === "emergency") {
    const emergencyRow = info ? (
      <div className="mi-emergency">
        <strong>🚑 {info.number}</strong>
        <span>
          {info.countryName} {info.desc}
        </span>
      </div>
    ) : (
      <div className="mi-emergency">
        <strong>🚑 112</strong>
        <span>국제 통합 긴급번호</span>
      </div>
    );
    return (
      <div>
        ...
        <div>
          {nearbyLoading ? (
            <div className="mi-row">
              <strong>📍 주변 응급시설</strong>
              <span className="mi-muted">조회 중...</span>
            </div>
          ) : (
            <div
              dangerouslySetInnerHTML={{ __html: nearbyHtml }}
              onClick={(e) => {
                if (e.target.id === "emergencyMapBtn" && emergencyMapUrl)
                  window.open(emergencyMapUrl, "_blank", "noopener");
              }}
            />
          )}
    ...
    return (
      <div>
        ...
        {amenitiesLoading && (
          <div className="mi-row">
            <strong>조회 중</strong>
            <span className="mi-muted">
              선택한 일정 주변 편의시설을 찾고 있습니다.
            </span>
          </div>
        )}
        ...
        {!amenitiesLoading &&
          !amenitiesError &&
          amenitiesData?.groups?.map((group) => {
  ...
  if (modalKey === "hotel")
    return (
      <div>
        ...
        >
          생성된 일정의 숙소 정보와 이동 동선을 기준으로 숙소 전략을 표시합니다.
        </p>
        <div className="mi-row">
          <strong>숙소 정보</strong>
          <span className="mi-muted">일정 데이터에서 확인 중</span>
        </div>
      </div>
    );
      ...
      <div className="bm-wrap">
        <div className="bm-hero">
          ...
          <div className="bm-hero-amounts">
            ...
            {total > 0 && (
              <span className="bm-hero-total">/ {formatKrw(total)}</span>
            )}
          </div>
          {total > 0 && (
        ...
        <div className="bm-stats">
          <div className="bm-stat-card">
            <span>남은 예산</span>
            <strong
              style={{ color: remaining > 0 ? "var(--green)" : "var(--muted)" }}
            >
  ...
  if (modalKey === "album")
    return (
      <div>
        ...
        >
          여행 중 저장한 사진과 메모. 종료 후 날짜별 타임라인으로 정리됩니다.
        </p>
        {albumPhoto ? (
          <div className="mi-album-preview">
            <img src={albumPhoto} alt="업로드된 사진" />
            <button
              className="mi-album-preview-del"
              onClick={() => {
                setAlbumPhoto("");
                setAlbumPhotoUrl("");
              }}
          ...
          <label className="mi-album-drop">
            ...
            <div className="mi-album-drop-text">
              클릭하거나 사진을 드래그하세요
            </div>
            <div className="mi-album-drop-hint">
              JPG · PNG · GIF · HEIC 지원
            </div>
          </label>
        )}
        <div className="mi-album-memo-wrap">
          <textarea
            className="mi-album-memo"
            maxLength={300}
            placeholder="오늘 여행 소감을 간단히 적어보세요..."
            value={albumMemo}
            onChange={(e) => setAlbumMemo(e.target.value)}
          />
          <div className="mi-album-memo-counter">
            <span>{albumMemo.length}</span> / 300
          </div>
        </div>
      </div>
    );
      ...
      <div>
        ...
        {!safetyLoading && safetyData && (
          <>
            <div
              className={
                safetyData.level === "warn" ? "mi-emergency" : "mi-safe"
              }
            >
              <strong>
        ...
        {!safetyLoading && !safetyData && (
          <div className="mi-row">
            <span>안전 우회 버튼을 눌러 실시간 사고 데이터를 조회하세요.</span>
          </div>
        )}
      </div>
    );
