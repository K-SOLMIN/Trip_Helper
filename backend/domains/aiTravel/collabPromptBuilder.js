...
2. 이동일(transfer day)은 오직 출발 숙소 출발 → 공항/역 이동 → 도착 도시 숙소 체크인으로만 구성합니다.
   - 이동일에는 관광 항목을 절대 추가하지 마세요.
...
function buildAustraliaCoverageSection(dest, days, styles, mustVisitList) {
  ...
  const minimumRegions = days >= 7 ? 3 : 2;
  return `\n[호주 권역 분산 원칙]\n목적지가 특정 도시가 아니라 호주 전체이고 mustVisit도 비어 있습니다. 시드니·멜버른만으로 전체 일정을 만들지 마세요.\n여행 스타일(${styles.join(', ') || '자유'})에 맞는 best 후보 권역을 먼저 고른 뒤, 권역별로 2~3일씩 머무는 허브형 일정으로 구성하세요.\n${days}일 일정에서는 최소 ${minimumRegions}개 이상 권역을 여행 흐름에 맞게 선택하세요.\n참고 후보: ${cityOptions.join(', ')}\n위 후보는 고정 목록이 아니라 RAG 참고 후보입니다. 날씨·예산·강도·mustVisit 위치·이동 효율을 비교해 더 적합한 권역이 있으면 바꿔도 됩니다.\n예: 관광+휴양이면 도시 관광 권역 1곳 + 해양/휴양 권역 1~2곳을 조합하세요.\n선택한 권역마다 숙소를 따로 두고, 장거리 이동일을 분리하세요.`;
}
...
- 인원: ${travelers}
${mustVisitLine}${multiCitySection}${australiaCoverageSection}${accSection}
