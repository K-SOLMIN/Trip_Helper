...
export const GOOGLE_MAP_SCRIPT_SRC =
  "https://maps.googleapis.com/maps/api/js?key=AIzaSyDaVmYg-OdmcaT1qDjLA-J-n5-df0XyWSw&callback=initMap&loading=async";
...
export function cleanPlaceName(name) {
  return String(name || "")
    .trim()
    .replace(
      /\s+(체크인|체크아웃|도착|출발|복귀|방문|관광|구경|산책|탐방)$/g,
      "",
    )
    .replace(/\s+/g, " ")
    .trim();
...
export function lookupEmergencyNumber(destination) {
  ...
  if (EMERGENCY_NUMBERS[dest])
    return { countryName: dest, ...EMERGENCY_NUMBERS[dest] };
  for (const [country, data] of Object.entries(EMERGENCY_NUMBERS)) {
    if (dest.includes(country) || country.includes(dest))
      return { countryName: country, ...data };
    if (data.aliases?.some((a) => dest.includes(a) || a.includes(dest)))
      return { countryName: country, ...data };
  }
  return null;
}
...
export function parseBudgetWon(value) {
  ...
  const unitTotal = units.reduce((sum, u) => {
    return (
      sum +
      [...compact.matchAll(u.pattern)].reduce(
        (s, m) => s + Number(m[1]) * u.multiplier,
        0,
      )
    );
  }, 0);
  if (unitTotal > 0)
    return unitTotal > MAX_BUDGET_WON ? 0 : Math.round(unitTotal);
...
export function getDayStops(stops) {
  ...
  const alreadyReturns = /숙소|hotel|Hotel|복귀/i.test(
    `${last.name || ""} ${last.badge || ""}`,
  );
...
export function getMapPoints(base, cityData) {
  ...
  const pts = {
    barcelona: [
      { lat: 41.3932, lng: 2.1699 },
      { lat: 41.3917, lng: 2.1649 },
      { lat: 41.3915, lng: 2.1686 },
      { lat: 41.3852, lng: 2.1809 },
      { lat: 41.3927, lng: 2.1587 },
      { lat: 41.395, lng: 2.1702 },
    ],
    madrid: [
      { lat: 40.4128, lng: -3.7002 },
      { lat: 40.4138, lng: -3.6921 },
      { lat: 40.4154, lng: -3.7089 },
      { lat: 40.4153, lng: -3.6844 },
      { lat: 40.4169, lng: -3.7035 },
    ],
    sevilla: [
      { lat: 37.3861, lng: -5.9928 },
      { lat: 37.3826, lng: -5.991 },
      { lat: 37.3891, lng: -5.9945 },
      { lat: 37.3761, lng: -5.9866 },
    ],
    granada: [
      { lat: 37.1773, lng: -3.5986 },
      { lat: 37.176, lng: -3.588 },
      { lat: 37.1808, lng: -3.6003 },
      { lat: 37.1722, lng: -3.5967 },
    ],
    malaga: [
      { lat: 36.7213, lng: -4.4217 },
      { lat: 36.7196, lng: -4.4189 },
      { lat: 36.7253, lng: -4.4176 },
      { lat: 36.7115, lng: -4.3923 },
    ],
    valencia: [
      { lat: 39.4699, lng: -0.3763 },
      { lat: 39.4539, lng: -0.3688 },
      { lat: 39.4736, lng: -0.379 },
      { lat: 39.456, lng: -0.3214 },
    ],
    bilbao: [
      { lat: 43.263, lng: -2.935 },
      { lat: 43.2592, lng: -2.9284 },
      { lat: 43.2604, lng: -2.9466 },
      { lat: 43.2556, lng: -2.9237 },
    ],
    sansebastian: [
      { lat: 43.3183, lng: -1.9812 },
      { lat: 43.3226, lng: -1.9753 },
      { lat: 43.3212, lng: -1.988 },
      { lat: 43.3099, lng: -2.0016 },
    ],
    zaragoza: [
      { lat: 41.6561, lng: -0.8773 },
      { lat: 41.6584, lng: -0.8752 },
      { lat: 41.6502, lng: -0.8831 },
      { lat: 41.6601, lng: -0.8705 },
    ],
    return: [
      { lat: 41.6561, lng: -0.8773 },
      { lat: 41.2971, lng: -0.917 },
    ],
  };
...
export function getRouteSegment(base, stopIdx, cityData) {
  ...
  return idx === 0
    ? [routePoints[0]]
    : [routePoints[idx - 1], routePoints[idx]];
}

// ── 이동수단 계산 ──────────────────────────────────────────────

export function transitPanelKey(day, stopIdx) {
  return `${day}-${stopIdx}`;
}
export function modeResultKey(day, stopIdx, mode) {
    ...
    return [
      {
        ...
        icon: "🚕",
        title: "택시",
        desc: taxiLive?.desc || "가까운 대로변 승차 · 야간 이동 우선",
        time: taxiLive?.time || "조회",
        minutes: taxiLive?.minutes ?? 999,
      },
      {
        mode: "walk",
        icon: "🚶",
        title: "도보",
        desc: walkLive?.desc || fallbackWalk.desc,
        time: walkLive?.time || fallbackWalk.time,
        minutes: walkLive?.minutes ?? fallbackWalk.minutes,
      },
      {
        mode: "transit",
        icon: "🚇",
        title: "대중교통",
        desc: transitLive?.desc || "정류장/역 기반 경로 조회",
        time: transitLive?.time || "조회",
        minutes: transitLive?.minutes ?? 999,
      },
    ];
  }
  return [
    ...
    },
    {
      ...
      icon: "🚇",
      title: "대중교통",
      desc: transitLive?.desc || "역/정류장 승하차 정보 조회",
      time: transitLive?.time || "조회",
      minutes: transitLive?.minutes ?? 999,
    },
    {
      ...
      icon: "🚕",
      title: "택시",
      desc: taxiLive?.desc || "가까운 승차 지점 호출 · 교통 상황 반영 예정",
      time: taxiLive?.time || "조회",
      minutes: taxiLive?.minutes ?? 999,
    },
  ];
...
export function getFastestMode(stop, i, state) {
  return getModeOptions(stop, i, state).reduce((best, opt) =>
    opt.minutes < best.minutes ? opt : best,
  );
}
...
export function getTransportPlan(stop, i, state) {
  ...
  return {
    rec: fastest.title,
    detail: fastest.time,
    options: getModeOptions(stop, i, state).map(
      (opt) => `${opt.title} ${opt.time}`,
    ),
  };
}

export function getTransitDemoOptions(
  stop,
  i,
  { day, transitResults, transitLoadingKey, routeModeResults },
) {
            ...
            detail: [
              {
                icon: "🚇",
                title: "대중교통 경로 조회 중",
                desc: "Google Transit 데이터로 승차 위치와 노선을 확인하고 있습니다.",
                time: "...",
              },
            ],
          }
  ...
  if (live)
    return getModeOptions(stop, i, state).map((opt) =>
      opt.mode === "transit" ? { ...opt, detail: live } : opt,
    );
  return getModeOptions(stop, i, state);
}

export function getTransitDetailOptions(
  stop,
  i,
  { day, transitResults, transitLoadingKey },
) {
  const key = transitPanelKey(day, i);
  if (transitLoadingKey === key) {
    return [
...
export function makeFormatters(exchangeRate) {
  ...
  const localToKrw = (amount) =>
    Math.round((Number(amount) || 0) * exchangeRate.rateToKrw);
...
export function getCurrentDaySpent(expenses, schedule, activeIdx) {
  return getCurrentDayExpenses(expenses, schedule, activeIdx).reduce(
...
export function getSpentThroughDay(expenses, dayNumber) {
  return expenses
    .filter((e) => Number(e.day || 1) <= dayNumber)
...
export function normalizeSavedExpense(expense) {
  ...
  const amountKrw = Number(
    expense.amountKrw ?? expense.amount_krw ?? amountLocal,
  );
  return {
    id: expense.id,
...
function knownPlaceCoordinates(name) {
  ...
  if (
    /달링하버\s*(레스토랑|식당|맛집)|darling harbour\s*(restaurant|dining)/.test(
      text,
    )
  )
...
export async function buildGeneratedTravelData() {
  ...
  if (planId) {
    try {
      result = (await readPlanResultFromDb(planId)) || fallbackResult;
    } catch (err) {
      console.warn("Failed to load travel plan from DB.", err);
    }
  }
  ...
  days.forEach((day, index) => {
    const stops = (day.items || []).map((item, itemIndex) => {
      ...
      return {
        ...
        name: escapeHtml(
          displayPlaceName(item.name || `일정 ${itemIndex + 1}`),
        ),
    ...
    cityData[`generated-${index}`] = {
      title: escapeHtml(
        day.theme || day.title || `${destination} ${index + 1}일차`,
      ),
    ...
    cityGroups: [
      {
        ...
        indices: days.map((_, i) => i),
      },
    ],
    activeIdx: 0,
    heroTitle: destination,
    routeText: days
      .map((day, i) => `${i + 1}일차 ${day.theme || day.title || ""}`.trim())
      .join(" → "),
...
export function formatTransitResult(result) {
  ...
  const options = leg.steps.map((step, idx) => {
    ...
    return {
      icon: "🚶",
      title: "도보 이동",
      desc: cleanStepText(step.instructions) || "정류장까지 이동",
      time: step.duration?.text || "",
      meta: [step.distance?.text || "", step.duration?.text || ""].filter(
        Boolean,
      ),
      route: stepRoute,
    };
  ...
  options.unshift({
    ...
    route: {
      ...
      path: leg.steps.flatMap(
        (s) => s.path?.map(latLngLiteral).filter(Boolean) || [],
      ),
...
export function requestTransitRoute(
  stopIdx,
  { schedule, activeIdx, cityData, day },
  onResult,
) {
  const s = schedule[activeIdx];
  if (!s) return;
      ...
      transitResult: [
        {
          icon: "ℹ",
          title: "대중교통 조회 불가",
          desc: "출발/도착지가 같은 기준점이거나 Google Maps가 아직 준비되지 않았습니다.",
          time: "-",
        },
      ],
    });
  ...
  svc.route(
    ...
    (result, status) => {
      const formatted =
        status === "OK" && result ? formatTransitResult(result) : null;
      const transitResult = formatted || [
        {
          icon: "ℹ",
          title: "대중교통 경로 없음",
          desc: `이 구간은 Google Transit 응답이 없습니다. 상태: ${status}`,
          time: "-",
        },
      ];
...
export function renderRouteMap(
  ...
  const modeKey =
    selectedTravelMode === "TAXI"
      ? "taxi"
      : selectedTravelMode === "WALKING"
        ? "walk"
        : "";
  const modeRoute = modeKey
    ? routeModeResults[modeResultKey(s.day, activeStopIdx, modeKey)]?.route
    : null;
  ...
  const routeFallback = () => {
    ...
    new google.maps.Polyline({
      path,
      map,
      strokeColor: selectedRoute ? "#F59E0B" : "#29ABE2",
      strokeOpacity: 0.82,
      strokeWeight: 4,
    });
  ...
  svc.route(
    {
      ...
      travelMode:
        selectedTravelMode === "TAXI"
          ...
          : google.maps.TravelMode[selectedTravelMode] ||
...
export function renderSafeRouteMap(
  targetId,
  {
    schedule,
    activeIdx,
  ...
  svc.route(
    ...
    (result, status) => {
      ...
      if (routes.length >= 2) {
        const origSec = routes[0].legs.reduce(
          (s, l) => s + (l.duration?.value ?? 0),
          0,
        );
        const safeSec = routes[1].legs.reduce(
          (s, l) => s + (l.duration?.value ?? 0),
          0,
        );
...
export async function fetchExchangeRate(destination) {
  if (!destination) return null;
  return apiGet(
    `/exchange-rate?destination=${encodeURIComponent(destination)}`,
  );
}
...
export async function fetchConsulateInfo(destination) {
  ...
  const res = await fetch(
    `${apiBase}/api/consulate?destination=${encodeURIComponent(destination)}`,
  );
...
export async function fetchEmergencyNearbyInfo(point, radius) {
  return apiGet(
    `/maps/emergency-places?lat=${point.lat}&lng=${point.lng}&radius=${radius}`,
  );
}

export async function fetchNearbyAmenities(point, radius = 1500) {
  return apiGet(
    `/maps/nearby-amenities?lat=${point.lat}&lng=${point.lng}&radius=${radius}`,
  );
}

export async function fetchIndoorPlaces(point, radius = 2000) {
  return apiGet(
    `/maps/indoor-places?lat=${point.lat}&lng=${point.lng}&radius=${radius}`,
  );
}

export async function fetchNearbyCafes(point, radius = 800) {
  return apiGet(
    `/maps/nearby-cafes?lat=${point.lat}&lng=${point.lng}&radius=${radius}`,
  );
}

export async function fetchDayWeather(destination, date) {
  return apiGet(
    `/weather/day?destination=${encodeURIComponent(destination)}&date=${encodeURIComponent(date)}`,
  );
}
