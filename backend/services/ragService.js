...
const CITY_MAP = {
  호주: ['시드니', '멜버른', '골드코스트', '케언즈', '울룰루', '브리즈번', '퍼스', '애들레이드'],
  시드니: ['시드니'],
  멜버른: ['멜버른'],
  골드코스트: ['골드코스트'],
  케언즈: ['케언즈'],
...
async function retrieveContext(query, { limit = 6, dest = null, budget = null, category = null } = {}) {
  if (dest && !CITY_MAP[dest]) return '';

  const city = resolveCityFilter(dest);
  const priceRange = budget ? PRICE_RANGE_MAP[budget] : null;
  const results = await searchKnowledge(query, { city, category, priceRange, limit });
